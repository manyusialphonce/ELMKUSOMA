...
export default function AdminRecordings() {
  ...
  const load = () => {
    adminApi.listRecordings({ status: status || undefined }).then(({ data }) => setRecordings(data.data)).catch(() => {});
  };
  ...
  return (
    <div>
      ...
      <div className="border border-gray-200 rounded-lg divide-y divide-gray-100">
        {recordings.map((r) => (
          <div key={r.id} className="p-3 text-sm flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-gray-400">{r.uploadedBy?.fullName} · {r.subject?.name}</p>
            </div>
        ...
        {recordings.length === 0 && <p className="text-sm text-gray-500 p-4">No recordings found.</p>}
      </div>
    </div>
  );
}
