  ...
  useEffect(() => {
    ...
    const fetchSchool = async () => {
      try {
        ...
        const { data } = await schoolsApi.getBySlug(slug);

        const apiSchool = data?.data || data;

        if (active && apiSchool) {
          setSchool(normalizeSchool(apiSchool));
        } else if (active) {
          setSchool(schoolDetails[slug] || null);
        }
      } catch (error) {
                ...
                <div className="rounded-xl bg-green-50 p-4">
                  ...
                  <p className="mt-1 text-2xl font-bold text-green-800">
                    ...
                    school.teachers !== null
                      ? Number(school.teachers).toLocaleString()
                      : '—'}
                  </p>
                </div>
              </div>
