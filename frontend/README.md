...
## Bug fix — nav links pointing to unbuilt routes

Several sidebar links (Student: My Quizzes, My Assignments, My Results,
Notifications, Profile, Notes Library; Teacher: Recordings, Students,
Questions, Resources, Performance, Subscription, Notifications, Profile;
Admin: most non-dashboard sections; Parent: Notifications, Profile) pointed
to routes that were never registered in the router. Clicking them threw a
real React Router error ("No routes matched location...") instead of
rendering a blank page, because a matched parent route with no matching
child route is treated as a routing error, not a silent miss.

Fixed two ways:
1. **`RouteErrorBoundary`** (`src/router/RouteErrorBoundary.jsx`) is now
   attached as `errorElement` on every top-level route group, so any future
   gap shows a friendly "This page isn't available yet" message instead of
   a blank crash.
2. **Every nav link now has a real matching route.** Fully built:
   `StudentQuizzes` (take quiz, auto-graded), `StudentAssignments` (submit),
   `StudentResults`, `TeacherRecordings`, `TeacherResources`,
   `TeacherPerformance`, `AdminUsers` (suspend/reactivate), `SchoolsList`,
   `SchoolDetail`. Shared across roles: `NotificationsPage`, `ProfilePage`
   (read-only — no backend update-profile endpoint exists yet, so this
   intentionally doesn't invent one), `SubscriptionPage`.
3. **Honestly stubbed** with `ComingSoon` (not fake data): Teacher →
   Students; Admin → Roles & Permissions, Regions, Education Levels,
   Classes, Subjects, Live Classes (admin-wide), Recordings (admin-wide),
   Resources (admin-wide), Quizzes (admin-wide), Assignments (admin-wide),
   Subscriptions, Payments, Reports (dashboard already covers this),
   Settings. These render a clear "being built next" message rather than
   silently failing.

Backend additions to support the above: `GET /recordings/mine`,
`GET /resources/mine` (teacher's own uploads including drafts),
`GET /quizzes/attempts/me` (student's own attempt history), and
`listQuizzes`/`listAssignments` now attach `myAttemptsCount`/`mySubmission`
for students. Also fixed: `userResource()` was silently dropping
`verificationStatus` from every auth response, which meant the "unverified
teacher" banners never had the data they needed.
